...
def training_with_ipex(args):
    ...
    if start_epoch == 0:  # and rank == 0
        save_model_path = os.path.join(model_dir, 'init.pt')
        save_checkpoint(optimized_model, save_model_path)

    # Start training loop
    for epoch in range(start_epoch, num_epochs):
        train_dataset.set_epoch(epoch)
        training_config['epoch'] = epoch
        lr = optimized_optimizer.param_groups[0]['lr']
        logging.info('Epoch {} TRAIN info lr {}'.format(epoch, lr))
        executor.train(optimized_model, optimized_optimizer, train_data_loader, device, writer,
                       training_config)
        cv_loss, cv_acc = executor.cv(optimized_model, cv_data_loader, device,
                                      training_config)
...
if __name__ == '__main__':
    args = get_args()
    if args.device == 'cuda':
        training_with_cuda(args)
    else:
        training_with_ipex(args)
